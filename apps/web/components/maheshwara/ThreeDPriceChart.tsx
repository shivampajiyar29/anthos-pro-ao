...
function Scene({ timeframe, symbol, currentPrice }: { timeframe: string; symbol: string; currentPrice: number }) {
    ...
    useEffect(() => {
        const config: Record<string, { count: number, vol: number }> = {
            '1m': { count: 30, vol: currentPrice * 0.0005 },
            '5m': { count: 30, vol: currentPrice * 0.001 },
            '1h': { count: 30, vol: currentPrice * 0.005 },
            '1d': { count: 30, vol: currentPrice * 0.02 },
        };
...
export default function ThreeDPriceChart({ symbol = 'BTCUSDT', currentPrice = 52000 }: { symbol?: string; currentPrice?: number }) {
    ...
    return (
        <div className="glass-panel w-full h-full relative overflow-hidden group min-h-[400px]">
            <div className="absolute top-6 left-6 z-10">
                <h3 className="section-label mb-1 !text-[var(--cyan)]">Holographic Price Engine</h3>
            ...
            <ErrorBoundary>
                <div className="w-full h-[400px]">
                    <Canvas camera={{ position: [0, 0, 15], fov: 40 }}>
                        <Scene key={`${symbol}-${timeframe}`} timeframe={timeframe} symbol={symbol} currentPrice={currentPrice} />
