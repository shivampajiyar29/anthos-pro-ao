                    ...
                    <div className="flex p-0.5 bg-black/40 rounded-lg border border-[var(--glass-border)]">
                        {agents.map((a) => (
                            <button
                                key={a}
                                onClick={() => setFilter(a)}
                                className={`px-2 py-1 rounded-md text-[9px] font-black transition-all ${filter === a ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                {a}
                            </button>
                        ))}
                ...
                <AnimatePresence mode="popLayout">
                    {displayTrades.map((trade, index) => (
                        <motion.div
                            ...
                            exit={{ opacity: 0, x: 20 }}
                            className="grid grid-cols-6 gap-4 p-3 rounded-lg bg-white/5 border border-white/5 hover:border-white/10 transition-all group items-center"
                        >
                    ...
                    {displayTrades.length === 0 && (
                        <div className="text-center py-20">
                            <p className="text-[10px] text-gray-600 font-black uppercase tracking-widest">Awaiting Neural Activity...</p>
                        </div>
                    )}
                </AnimatePresence>
            </div>
